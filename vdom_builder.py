...
class Builder(object):
    ...
    def write_xml(self, tagname, attrs=None, data=None, close=False, indent=0, force_cdata=False, closing=False):
    ...
    def write_e2vdom(self):
        ...
        for event in all_events:
            ...
            for action in actions:
                self.write_xml("Action", attrs={"ID": action}, indent=8, data="", close=True)
        ...
        for action in all_actions:
            ...
            for param in params:
                self.write_xml("Parameter", attrs={"ScriptName": param[0]}, indent=8, data=param[1], close=True)
    ...
    def write_libraries(self):
        ...
        for lib_name in os.listdir(libs_path):
            ...
            with self.open_file(p) as lib_f:
                self.write_xml(
                    tagname="Library", attrs={"Name": lib_name.split(".")[0]}, indent=4, data=lib_f.read(), close=True)
    ...
    def write_resources(self):
        ...
        for res_name in os.listdir(resources_path):
            ...
            with self.open_file(p) as res_f:
                self.write_xml(
                    tagname="Resource", attrs=attrs, indent=4, data=base64.b64encode(res_f.read()), close=True)
    ...
    def write_databases(self):
        ...
        for db_name in os.listdir(dbs_path):
            ...
            with self.open_file(p) as db_f:
                self.write_xml(
                    tagname="Database", attrs=attrs, indent=4, data=base64.b64encode(db_f.read()), close=True)
        ...
        for user in users_json:
            ...
            for key, value in user.items():
                if key == "Rights":
                    ...
                    for right in value:
                        self.write_xml("Right", attrs=right, indent=10, close=True)
                    self.write_xml("Rights", indent=8, closing=True)

                else:
                    self.write_xml(key, data=value, indent=8, close=True, force_cdata=True)
    ...
    def write_actions(self, path, indent):
        ...
        for action_name in os.listdir(path):
            ...
            with self.open_file(p) as action_f:
                self.write_xml(
                    tagname="Action", attrs=attrs, indent=indent+2, data=action_f.read(), close=True, force_cdata=True)
    ...
    def write_attributes(self, attributes, indent):
        ...
        for key, value in attributes.items():
            self.write_xml("Attribute", attrs={"Name": key}, data=clear_data(encode(value)), indent=indent+2, close=True)            
